...
import { Heart, PartyPopper, Moon, Music, Wand2, Copy, Check, Puzzle, Palette, Image as ImageIcon, Clock, Mic, Square, Gift, Sparkles, Save } from 'lucide-react';
...
export function CardEditor({ initialData, onPreview, onSaveOnly }: CardEditorProps) {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
            ...
            if (responseData.secure_url) {
              ...
              toast('Voice note uploaded successfully! ☁️', 'success');
            } else {
  ...
  return (
    <div className="flex-1 flex items-center justify-center py-6 sm:py-12 px-4 sm:px-6 lg:px-8 bg-transparent">
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        className="max-w-2xl w-full space-y-6 sm:space-y-8 bg-white/40 backdrop-blur-xl border border-white/60 p-6 sm:p-10 rounded-3xl shadow-2xl relative"
      >
        <div className="relative mb-6 flex flex-col items-center">
          <div className="w-full flex justify-end mb-2">
            {localStorage.getItem(AUTOSAVE_KEY) && (
              <button
                ...
                className="text-[10px] text-gray-400 hover:text-red-500 font-bold tracking-wider uppercase transition-colors"
          ...
          <h2 className="text-center text-3xl font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-pink-500 to-blue-500 tracking-tight">
          ...
          <div className="flex gap-2 items-center mt-2">
            ...
            <span className="text-[10px] bg-green-100 text-green-600 px-1.5 py-0.5 rounded font-bold uppercase tracking-widest hidden sm:inline-block">Auto-saving</span>
          </div>
        </div>
        ...
        <div className="space-y-6 relative">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            <div>
              <label className="text-xs font-bold text-gray-500 mb-2 block uppercase tracking-widest">To</label>
              <input
                type="text"
                value={data.to}
                onChange={e => setData({ ...data, to: e.target.value })}
                className="block w-full bg-white/60 border border-white/80 rounded-2xl shadow-sm focus:ring-2 focus:ring-pink-300 p-3 text-sm text-gray-700 outline-none backdrop-blur-sm transition-all"
                placeholder="Recipient's Name"
              />
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 mb-2 block uppercase tracking-widest">From</label>
              <input
                type="text"
                value={data.from}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                ...
                <select
                  ...
                  className="bg-white/50 border border-white/80 rounded-lg text-xs font-bold text-gray-700 p-1 outline-none focus:ring-2 focus:ring-pink-300"
                >
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs font-bold text-pink-500 bg-pink-50 hover:bg-pink-100 py-1.5 px-3 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
            ...
            <textarea
              ...
              className={cn(
                "block w-full bg-white/60 border border-white/80 rounded-2xl shadow-sm focus:ring-2 focus:ring-pink-300 p-4 text-sm text-gray-700 outline-none backdrop-blur-sm transition-all custom-scrollbar resize-none h-32",
                data.fontFamily === 'sans' && "font-sans text-base",
                data.fontFamily === 'serif' && "font-serif text-lg",
                data.fontFamily === 'dancing' && "font-dancing text-2xl",
                data.fontFamily === 'pacifico' && "font-pacifico text-xl",
                (data.fontFamily === 'handwriting' || !data.fontFamily) && "font-handwriting text-xl"
              )}
              placeholder="Write a sweet message..."
            />
            <div className="mt-3 flex items-center justify-between p-3 bg-white/40 border border-white/60 rounded-xl">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-pink-100 flex items-center justify-center text-pink-500 shrink-0">
                   <Mic className="w-4 h-4" />
                </div>
                <div>
                   <span className="text-xs font-bold text-gray-700 block">Voice Message</span>
                   <span className="text-[10px] text-gray-500 font-medium hidden sm:block">Record a personalized audio message</span>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {isRecording ? (
                  <button onClick={stopRecording} className="flex items-center gap-1 px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-full text-xs font-bold transition-colors shadow-sm animate-pulse">
                    <Square className="w-3 h-3 fill-white" /> Stop
                  </button>
                ) : (
                  <button onClick={startRecording} className="flex items-center gap-1 px-3 py-1.5 bg-pink-500 hover:bg-pink-600 text-white rounded-full text-xs font-bold transition-colors shadow-sm">
                    <Mic className="w-3 h-3" /> Record
                  </button>
                )}
                {data.recordedAudio && !isRecording && (
                   <button onClick={() => { setData({...data, recordedAudio: undefined}); setCopied(false); }} className="p-1.5 text-gray-400 hover:text-red-500 bg-white rounded-full shadow-sm transition-colors" title="Delete Recording">
            ...
            {data.recordedAudio && !isRecording && (
              <div className="mt-2 text-center">
                 <audio controls src={data.recordedAudio} className="h-8 w-full max-w-[200px] mx-auto" />
              </div>
            )}
          </div>

          <div>
            <label className="text-xs font-bold text-gray-500 mb-3 block uppercase tracking-widest">Theme</label>
            <div className="grid grid-cols-3 gap-4">
              {themes.map((t) => (
                ...
                >
                  <div className={cn("mb-2 drop-shadow-sm", t.color)}>
                    {t.icon}
                  </div>
                  <span className={cn("text-xs font-bold", data.theme === t.id ? "text-pink-600" : "")}>{t.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            <label className="text-xs font-bold text-gray-500 block uppercase tracking-widest flex items-center gap-2">
              <Palette className="w-4 h-4" /> Card Cover Color
            </label>
            <div className="flex flex-wrap gap-3">
              ...
              <div 
                className={cn(
                  "relative w-10 h-10 rounded-full border-2 transition-all overflow-hidden shadow-sm flex items-center justify-center bg-gray-100",
              ...
              >
                 <input 
                   type="color" 
                   className="absolute inset-0 w-20 h-20 -top-5 -left-5 cursor-pointer"
          ...
          <div>
             <label className="text-xs font-bold text-gray-500 mb-3 block uppercase tracking-widest">Music Tune</label>
             <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
               {musicOptions.map((opt) => (
                 <button
                   ...
                   className={cn(
                     "p-3 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 duration-300",
                     data.music === opt.id
                       ? "bg-pink-100/50 border-pink-400 text-pink-600 shadow-sm"
                       : "bg-white/40 border-transparent text-gray-500 hover:bg-white/60"
                   )}
                 >
                   <Music className={cn("w-5 h-5 drop-shadow-sm", data.music === opt.id ? "opacity-100" : "opacity-50")} />
                   <span className="text-[10px] font-bold uppercase tracking-wider text-center leading-tight">{opt.label}</span>
                 </button>
               ))}
             </div>
             {data.music === 'custom' && (
               <div className="mt-3 p-4 bg-white/40 border border-white/60 rounded-2xl animate-in fade-in slide-in-from-top-2 space-y-4">
                 <div>
                   <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">Paste a YouTube or Audio Link</label>
                   <input
                     type="text"
                     ...
                     className="block w-full bg-white/60 border border-white/80 rounded-xl shadow-sm focus:ring-2 focus:ring-pink-300 p-3 text-sm text-gray-700 outline-none backdrop-blur-sm transition-all"
                 ...
                 <div>
                   <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Upload Audio (&lt; 3MB for sharing)</label>
                   <input
                     type="file"
                     accept="audio/*"
                     onChange={(e) => {
                     ...
                     className="block w-full text-xs text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-pink-100 file:text-pink-600 hover:file:bg-pink-200 file:transition-colors file:cursor-pointer cursor-pointer"
                   />
                 </div>
               </div>
             )}
          ...
          <div>
             <label className="text-xs font-bold text-gray-500 mb-3 block uppercase tracking-widest flex items-center gap-2">
               <ImageIcon className="w-4 h-4" /> Surprise Photo Inside
             </label>
             <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
               {photoOptions.map((opt) => (
                 <button
                   ...
                   className={cn(
                     "p-3 rounded-2xl border-2 transition-all flex flex-col items-center justify-center gap-2 duration-300",
             ...
             {data.surprisePhoto === 'custom' && (
               ...
               >
                 <div>
                   <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2">Upload Photos (&lt; 3MB total for sharing)</label>
                   <input 
                     type="file" 
                     accept="image/*"
                     multiple
                         ...
                         const processFiles = async () => {
                           ...
                           for (const file of files) {
                             ...
                             if (!cloudName || !uploadPreset) {
                               ...
                               formData.append('upload_preset', uploadPreset);
                               const toastId = toast('Uploading image...', 'info');
                               try {
          ...
          <div className="flex flex-col gap-2 mt-4 relative z-10">
             <div className="flex flex-col gap-2 mb-2">
                <label className="text-xs font-bold text-gray-500 mb-1 block uppercase tracking-widest flex items-center gap-2">
                  <Wand2 className="w-4 h-4" /> Floating Effects
                </label>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white/40 border border-white/60 rounded-2xl transition-all hover:bg-white/60 gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-indigo-400 to-blue-400 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0">
                  <Clock className="w-5 h-5" />
                </div>
                <div>
                  <span className="text-sm font-bold text-gray-700 block">Date/Time Lock</span>
                  <span className="text-xs text-gray-500 font-medium sm:block">Lock card until a specific time</span>
                </div>
              </div>
              <div className="flex flex-col items-end w-full sm:w-auto">
                <input
                  ...
                  className="w-full sm:w-auto bg-white/70 border-2 border-white/80 focus:border-indigo-400 rounded-xl px-3 py-1.5 outline-none transition-all text-xs font-medium text-gray-700 min-w-0 sm:min-w-[180px]"
                />
                {data.unlockDate && (
                  <button
                    onClick={() => {
                    ...
                    className="text-[10px] text-red-400 mt-1 hover:text-red-600 font-medium"
                  >
                    Clear Lock
                  </button>
                )}
            ...
            <div className="flex items-center justify-between p-4 bg-white/40 border border-white/60 rounded-2xl transition-all hover:bg-white/60">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-400 to-purple-400 rounded-xl flex items-center justify-center text-white shadow-sm">
                  <Puzzle className="w-5 h-5" />
                </div>
                <div>
                  <span className="text-sm font-bold text-gray-700 block">Multi-Page Puzzle</span>
                  <span className="text-xs text-gray-500 font-medium hidden sm:block">Require solving fun puzzles before opening</span>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-white/40 border border-white/60 rounded-2xl hover:bg-white/60 transition-all cursor-pointer" onClick={() => setData({ ...data, enableInteractiveUnwrap: !data.enableInteractiveUnwrap })}>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-purple-400 to-pink-400 rounded-xl flex items-center justify-center text-white shadow-sm shrink-0">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-gift w-5 h-5"><rect x="3" y="8" width="18" height="4" rx="1"/><path d="M12 8v13"/><path d="M19 12v7a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2v-7"/><path d="M7.5 8a2.5 2.5 0 0 1 0-5A4.8 8 0 0 1 12 8a4.8 8 0 0 1 4.5-5 2.5 2.5 0 0 1 0 5"/></svg>
                </div>
                <div>
                  <span className="text-sm font-bold text-gray-700 block">Interactive Unwrap</span>
                  <span className="text-xs text-gray-500 font-medium hidden sm:block">Recipient taps to tear open a virtual gift box</span>
                </div>
              </div>
              
              <button
                onClick={(e) => { e.stopPropagation(); setData({ ...data, enableInteractiveUnwrap: !data.enableInteractiveUnwrap }); }}
                className={cn("w-12 h-6 rounded-full p-1 transition-all duration-300", data.enableInteractiveUnwrap ? "bg-pink-400" : "bg-gray-300")}
              >
                <div className={cn("w-4 h-4 bg-white rounded-full transition-transform duration-300 shadow-sm", data.enableInteractiveUnwrap ? "translate-x-6" : "translate-x-0")}></div>
              </button>
            </div>
          ...
          <div className="pt-6 space-y-4">
            ...
            <button
              ...
              className="w-full flex justify-center items-center gap-2 py-4 px-6 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-2xl text-base font-bold shadow-lg shadow-pink-200/50 hover:shadow-xl hover:scale-[1.02] transition-all"
            >
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="flex justify-center items-center gap-2 py-3 px-4 bg-white/70 backdrop-blur-sm border-2 border-pink-200 text-pink-600 rounded-xl text-sm font-bold hover:bg-pink-50 hover:border-pink-300 transition-all"
              >
              ...
              <button
                onClick={handleCopy}
                className="flex justify-center items-center gap-2 py-3 px-4 bg-white/70 backdrop-blur-sm border-2 border-blue-200 text-blue-600 rounded-xl text-sm font-bold hover:bg-blue-50 hover:border-blue-300 transition-all"
              >
